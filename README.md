...
**⚠️ grade(등급) 규칙 — 절대 빈칸 금지:**
- 모든 문항은 반드시 `S/A/B/C/기/인/소/Z` 중 하나의 등급을 가져야 한다. `""`(빈칸) 금지.
- 의미: S/A/B/C = 중요등급(높→낮), 기 = 기본, 인 = 인사처, 소 = 소방, Z = 지엽.
- 원본 책에서 등급은 페이지 **좌측 세로 괄호(bracket)로 여러 문항을 묶어** 한 번만 표기된다.
  괄호가 덮는 범위의 모든 문항에 같은 등급을 부여한다.
- 추출 시 문항 번호 왼쪽 여백을 확대해 괄호 라벨을 반드시 판독할 것. 불확실하면 직전 그룹 등급 상속 또는 "기".
- 배포 전 검증: `python3 -c "import json,glob; [print(f) for f in glob.glob('public/chapters/*.json') if any(not q.get('grade') for q in json.load(open(f))['questions'])]"` 로 빈 등급 챕터가 없는지 확인.
